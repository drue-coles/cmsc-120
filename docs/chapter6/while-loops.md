...
Incrementing or decrementing an `int` variable by one is a common pattern, especially in loops. 
Compound assignment operators can be used for this, but Java provides an even simpler syntax with 
...
The increment and decrement operators, compound assignment operators, and the ternary conditional
operator are all examples of what programmers call **syntactic sugar**: they do not provide any new
capabilities, but they do provide a convenient shorthand for existing syntax.
